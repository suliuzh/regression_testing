...
class CallGraphVisitor(ast.NodeVisitor):
    ...
    def postprocess(self):
        ...
        self.expand_unknowns()        #增加了一些错误的模块（名称相似，没有命名空间）
        self.contract_nonexistents()  #没有定义的情况(这一步用来过滤掉上一步出错的情况，同时也过滤掉import的情况)
        self.cull_inherited()         #有传递关系的模块
        # self.collapse_inner()   #
...
#        self.last_value = self.get_node('', tn, node)
                ...
                if from_node.flavor in [Flavor.FUNCTION, Flavor.METHOD, Flavor.STATICMETHOD, Flavor.CLASSMETHOD]:
                    try:
                        ...
                        if spl_to_node_str[0] == 'numpy':
                            ...
                            # print(to_node.name)
                            #将其导出来
                            if from_node not in self.cross_edge.keys():
                                self.cross_edge[from_node]=set()
                                self.cross_edge[from_node].add(to_node)
                            else:
                                self.cross_edge[from_node].add(to_node)
    ...
    def analyze_generators(self, generators):
        ...
        for gen in generators:
            # TODO: there's also an is_async field w
            # e might want to use in a future version.
